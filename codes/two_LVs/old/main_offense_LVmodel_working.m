...
filename = 'offense_r_gamma_LBinduction.mat';
% filename = 'offense_r_gamma_noLBinduction.mat';
...
dt = 0.5; % hours
lag_time_LB = 1; % point of introduction of LB
t_end = 200; % hours
t_span1 = transpose(0:dt:t_end); % time
t_span2 = transpose((lag_time_LB+dt):dt:t_end); % time
...
for count = 1:2 %:length(gam_vals_range)
    ...
    if count == 1
        ...
        [t_traj,y_traj] = ode45(@(t,y)simulate_2LVmodel(t,y,params), t_span1, init_conds, options);
    
    else
            
        init_conds = y_traj(end,:);
        init_conds(3) = 1;%y_traj(end,3)+1e-3;
    
        [t_traj,y_traj] = ode45(@(t,y)simulate_2LVmodel(t,y,params), t_span2, init_conds, options);
    end
...
%     params.r_B = this_r_B;
    ...
    LA_traj = [LA_traj,this_LA_traj];
    VA_traj = [VA_traj,this_VA_traj];
    LB_traj = [LB_traj,this_LB_traj];
    VB_traj = [VB_traj,this_VB_traj];
...
%% track LA,VA up to VA increasing
VB_traj_shift = circshift(VB_traj(1,:),1);

[val ind] = find(VB_traj_shift(1,:) - VB_traj(1,:) > 0);
VB_traj_VBincreasing = VB_traj(1:ind(2));
VA_traj_VBincreasing = VA_traj(1:ind(2));
LA_traj_VBincreasing = LA_traj(1:ind(2));
...
% VB_vals_range = 10.^linspace(-6,-3,200);
...
f2 = figure(2); set(f2, 'Position', [200 800 600 900]);

subplot(2,1,1);
lg(1) = semilogy(t_span,LA_traj,'Color',[0.5 0.5 0.5],'linewidth',4); hold on;
lg(2) = semilogy(t_span,VA_traj,'Color',my_rgb_colors(3,:),'linewidth',4); hold on;
lg(3) = semilogy(t_span,LB_traj,'k','linewidth',4); hold on;
lg(4) = semilogy(t_span,VB_traj,'Color',my_rgb_colors(2,:),'linewidth',4); hold on;
semilogy(lag_time_LB*ones(1,10),10.^linspace(0,10,10),'k--','linewidth',3); hold on;
% time_pt 
...
legend_char1 = ['Resident Lysogen, $L_A$'];
legend_char2 = ['Resident Phage, $V_A$'];
legend_char3 = ['Invading Lysogen, $L_B$'];
legend_char4 = ['Invading Phage, $V_B$'];

legend(lg,{legend_char1,legend_char2,legend_char3,legend_char4}, 'Interpreter','Latex','Location','SouthEast','FontSize',13);
...
% h(1) = plot(t_span,r_A*ones(size(VB_traj_VBincreasing)),'Color',[0.5 0.5 0.5],'linewidth',4); hold on;
% plot(gam_A,r_A,'.','Color',[0.5 0.5 0.5],'MarkerSize',50);
% [val ind]=find(gam_vals_range>10^-4);
% h(2) = plot(t_span,r_corresponding_lysogenicR0equalsone(ind(1))*ones(size(VB_traj_VBincreasing)),'k--','linewidth',4); hold on;
plot(t_span,r_corresponding_lysogenicR0equalsone_withVB,'Color',my_rgb_colors(1,:),'linewidth',4); hold on;
plot(lag_time_LB*ones(1,10),linspace(0,1.05,10),'k--','linewidth',3); hold on;
% plot(gam_vals_range,r_corresponding_tradeoff,'Color',[0.5 0.5 0.5],'linewidth',2); hold on;

% legend_char1 = ['Growth rate of Resident Lysogen, $r_A = 1$'];
% legend_char2 = ['Invading Lysogen, $\hat r_B$ (no $V_B$)'];
% legend_char3 = ['Growth Requirement of Invading Lysogen, $\tilde r_B (V_B(t))$'];

% legend(h,{legend_char1,legend_char2,legend_char3}, 'Interpreter','Latex','Location','NorthWest','FontSize',12);
...
ylim([0 1.05]);
% axis([x_values(1) x_values(end) 0 2*10^5]);
xlabel('Lag Time, $\tau_{intro}$ (hr)','interpreter','latex');
ylabel('Minimal Growth Rate, $\hat r_B$ (hr$^{-1}$)','interpreter','latex');
