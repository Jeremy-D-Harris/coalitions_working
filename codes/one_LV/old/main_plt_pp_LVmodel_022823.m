...
gamma = 1e-5; % lysis rate (per hour)
bet = 10; % burst size
...
init_conds_range = [3e8,1e6,2e5,3e9;2.5e5,1e5,6e3,2e4];
...
%% nullclines
x_values = linspace(10^2,1e6,100);%10.^linspace(1,5,100);
y_values = linspace(10^5,10^10,100);%10.^linspace(1,5,100);

Lysogen_equilibrium_zero = zeros(size(x_values));
Lysogen_equilibrium_nonzero = (K*(r-gamma-d)/r);
Lysogen_equilibrium_nonzero_vector = Lysogen_equilibrium_nonzero*ones(size(x_values));
phage_nullcline = m*x_values./(bet*gamma-phi*x_values);%bet*eta*x_values./(phi*x_values+m);
phage_equilibrium_nonzero = bet*gamma*Lysogen_equilibrium_nonzero/(phi*Lysogen_equilibrium_nonzero+m);
phage_nullcline_asymptote = bet*gamma/phi;
...
loglog(V_traj(2:end,:)',L_traj(2:end,:)','linewidth',2); hold on;
...
legend(h,{legend_char1,legend_char2,legend_char3,legend_char4}, 'Interpreter','Latex','Location','NorthWest','FontSize',16);
