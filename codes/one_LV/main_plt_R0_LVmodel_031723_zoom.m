...
figure_name = 'LV_R0_gammaA_rB_noheatmap_zoom.eps';
...
plot(gam_vals_range,r_A_corresponding,'k','linewidth',4); hold on;
plot(gam_A,r_A,'k.','MarkerSize',50);
...
plot(gam_vals_range,r_A_corresponding,'k','linewidth',3);
plot(gam_A,r_A,'k.','MarkerSize',40);
set(gca,'YDir','normal');
cb1 = colorbar;
xlim([1e-6 1e-2]);
ylim([0.95 1.05]);
...
% plot(gam_vals_range,r_corresponding_tradeoff,'k','linewidth',4); hold on;
% plot(gam_A,r_A,'k.','MarkerSize',50);
if isempty(ind)~=1
...
%  cb1.Ticks = linspace(0.98, 1.02, 9); 
%  cb1.TickLabels = [{'<0.98'},{''},{'0.99'},{''},{'1'},{''},{'1.01'},{''},{'1.02'}];   
...
% ylabel('Lysogen Growth Rate, $r_B$ (hr$^{-1}$)','interpreter','latex');
...
% set(gca,'YDir','normal');
% cb2 = colorbar;
% % cb2.Ticks = linspace(1, 4, 7); 
% % cb2.TickLabels = [{'<1'},{''},{'2'},{''},{'3'},{''},{'4'}];   
% % cb1.Label.String = 'CFU/mL';
% xlim([1e-6 1e-3]);
% ylim([r_A-updown_amt r_A+updown_amt]);
% % axis([x_values(1) x_values(end) 0 2*10^5]);
