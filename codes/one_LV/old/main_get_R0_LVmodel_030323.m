...
filename = 'LV_reproductionnumber_r_gamma.mat';
...
r_vals_range = linspace(0.95,1.05,100);
gam_vals_range = 10.^linspace(-6,0,100);
...
phage_equilibrium_fixed = bet*gam_fixed*lysogen_equilibrium_fixed/(m);
% phage_equilibrium_fixed = bet*gam_fixed*lysogen_equilibrium_fixed/(phi*lysogen_equilibrium_fixed+m);

lysogen_reproduction_number = zeros(length(r_vals_range),length(gam_vals_range));
...
r_fixed_corresponding =  (gam_vals_range + d)/(1-lysogen_equilibrium_fixed/K);

for count_gam = 1:length(gam_vals_range)
    
    this_gam = gam_vals_range(count_gam);
    this_r = r_fixed_corresponding(count_gam);
    
    this_phage_equilibrium = bet*gam_fixed*lysogen_equilibrium_fixed/(phi*lysogen_equilibrium_fixed+m);
    phage_equilibrium_corresponding = bet*this_gam*lysogen_equilibrium_fixed/(phi*lysogen_equilibrium_fixed+m);
    
    %         lysogen_equilibrium_fixed = max((K*(this_r-this_gam-d)/this_r),10^0);
    this_lysogen_reproduction_number_lytic = bet*lysogen_equilibrium_fixed*phi/(phi*lysogen_equilibrium_fixed+m);
    lysogen_reproduction_number_lytic(count_gam) = this_lysogen_reproduction_number_lytic;
%     this_lysogen_reproduction_number_lysogenic = this_r*(1-lysogen_equilibrium_fixed/K)/(d+this_gam+phi*phage_equilibrium_corresponding);
    this_lysogen_reproduction_number_lysogenic = 1.5*(1-lysogen_equilibrium_fixed/K)/(d+this_gam+phi*phage_equilibrium_fixed);
    lysogen_reproduction_number_lysogenic(count_gam) = this_lysogen_reproduction_number_lysogenic;
    
    this_lysogen_reproduction_number = max(this_lysogen_reproduction_number_lytic,this_lysogen_reproduction_number_lysogenic);
    lysogen_reproduction_number(count_gam) = this_lysogen_reproduction_number;
%     phage_equilibrium(count_r,count_gam) = bet*this_gam*lysogen_equilibrium_fixed/(phi*lysogen_equilibrium_fixed+m);
...
%     subplot(1,2,1);
%     imagesc(gam_vals_range,r_vals_range,max(lysogen_reproduction_number,10^0)); hold on;
    plot(gam_vals_range,lysogen_reproduction_number_lytic,'Color',my_rgb_colors(1,:),'linewidth',2); hold on;
    plot(gam_vals_range,lysogen_reproduction_number_lysogenic,'Color',my_rgb_colors(2,:),'linewidth',2); hold on;
    plot(gam_vals_range,lysogen_reproduction_number,'k--','linewidth',2); hold on;
