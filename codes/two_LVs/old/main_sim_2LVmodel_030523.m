...
r_B = 1.005; % growth rate (per hour)
gam_B = 1e-4; % lysis rate (per hour)
...
f1 = figure(1); set(f1, 'Position', [10 500 1850 450]);
subplot(1,3,1);
% semilogy(x_values,Lysogen_equilibrium_zero,'Color',my_rgb_colors(1,:)); hold on;
h(1)=loglog(x_values_pnull,Lysogen_equilibrium_nonzero_vector,'--','Color',my_rgb_colors(2,:),'linewidth',1); hold on;
...
loglog(VA_traj,LA_traj,'k','linewidth',2); hold on;
...
h(3)=loglog(VA_traj(1,end),LA_traj(1,end),'ko','MarkerSize',9,'linewidth',2); hold on;
h(4)=loglog(phage_equilibrium_nonzero,Lysogen_equilibrium_nonzero,'k.','MarkerSize',30); hold on;
xlim([10^0 10^10]);
ylim([10^0 10^10]);
xlabel('Resident Virus, $V_A$ (PFU$/$mL)','interpreter','latex');
ylabel('Resident Lysogen, $L_A$ (Cells$/$mL)','interpreter','latex');
f1=gca;
f1.LineWidth = 1;
f1.FontSize = 24;
...
% h(2)=loglog(x_values,phage_nullcline,'--','Color',my_rgb_colors(1,:),'linewidth',1); hold on;
% loglog(phage_nullcline_asymptote*ones(size(y_values)),y_values,'k--','linewidth',1); hold on;
% plot(fraction_VB,fraction_LB,'k','linewidth',2); hold on;
loglog(VB_traj,LB_traj,'Color',my_rgb_colors(3,:),'linewidth',2); hold on;
% loglog(VA_traj(3,:),LA_traj(3,:),'Color',default_rgb_colors(3,:),'linewidth',2); hold on;
% loglog(VA_traj(4,:),LA_traj(4,:),'Color',default_rgb_colors(2,:),'linewidth',2); hold on;
plot(VB_traj(1,1),LB_traj(1,1),'k.','MarkerSize',20); hold on;
h(3)=loglog(VB_traj(1,end),LB_traj(1,end),'ko','MarkerSize',9,'linewidth',2); hold on;
% loglog(phage_equilibrium_nonzero,Lysogen_equilibrium_nonzero,'k.','MarkerSize',30); hold on;

xlim([10^0 10^10]);
ylim([10^0 10^10]);
% axis([x_values(1) x_values(end) 0 2*10^5]);
xlabel('Invading Virus, $V_B$ (PFU$/$mL)','interpreter','latex');
ylabel('Invading Lysogen, $L_B$ (Cells$/$mL)','interpreter','latex');
f1=gca;
f1.LineWidth = 1;
f1.FontSize = 24;
f1.FontName = 'Times New Roman';
...
% plot L,V over time
subplot(1,3,3);
g(1)=semilogy(t_traj,LA_traj(1,:),'Color',default_rgb_colors(1,:),'linewidth',2); hold on;
g(2)=semilogy(t_traj,VA_traj(1,:),'--','Color',default_rgb_colors(1,:),'linewidth',2); hold on;
g(3)=semilogy(t_traj,LB_traj(1,:),'Color',my_rgb_colors(3,:),'linewidth',2); hold on;
g(4)=semilogy(t_traj,VB_traj(1,:),'--','Color',my_rgb_colors(3,:),'linewidth',2); hold on;
% semilogy(t_span,LA_traj(3,:),'Color',default_rgb_colors(3,:),'linewidth',2); hold on;
% semilogy(t_span,VA_traj(3,:),'--','Color',default_rgb_colors(3,:),'linewidth',2); hold on;
% semilogy(t_span,LA_traj(4,:),'Color',default_rgb_colors(2,:),'linewidth',2); hold on;
% semilogy(t_span,VA_traj(4,:),'--','Color',default_rgb_colors(2,:),'linewidth',2); hold on;
